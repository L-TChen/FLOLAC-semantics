...
\begin{frame}{Denotational semantics of \PCF}
  Instead of specifying \emph{how} a program runs, we specify \emph{what} a
  program is, the \emph{denotation} of a program. 
  \\~\\
  To assign a denotation to a program, 
  \begin{itemize}
    \item each type $\sigma$ is interpreted as some domain $D_\sigma$;
  ...
  ~\\
  In the end, we show that $(\lambda x.\, \M)\;\N$ and $\lambda x.\, \M\; x$
  have the same denotation as $\M[\N/x]$ and $\M$ respectively, and also the
  Compactness Theorem for the Scott domain model of \PCF.
\end{frame}
  ...
  \begin{description}
    \item[(Y)] The denotation of $\fix$ is the fixpoint operation 
      \[
        ...
        \dfix
        \left(
          \sem{\Gamma, x : \sigma \vdash \M : \sigma}\;\vec{d}
        \right)
      \]
    ...
    \item[(ifz)] The denotation of $\ifz$ 
      \begin{align*}
        & \sem{\Gamma \vdash \ifz(\M; \M_0; \M_1) : \tau}\;\vec{d} \\
        \defeq{} & \mathit{ifz}_\tau (n , d, f)
      \end{align*}
      where
      \begin{enumerate}
        \item $n \defeq \sem{\Gamma \vdash \M  : \nat}\;\vec{d}$, 
        \item $d \defeq \sem{\Gamma \vdash \M_0 : \tau}\;\vec{d}$, 
        \item $f \defeq \sem{\Gamma, x : \nat \vdash \M_1 : \tau}\; \vec{d}$,
      \end{enumerate} and $\mathit{ifz}_\tau$ is defined by
          \[
...
\begin{frame}{Substitution Lemma}
  \begin{lemma}
    ...
    \[
      \sem{\Delta \vdash \M[\vec{N}/\vec{x}] : \tau}\; \vec{d}
  ...
  \end{lemma}
  \begin{proof}
    We prove it by induction on derivations of $\Gamma \vdash \M : \tau$. 
  \end{proof}
\end{frame}
...
\begin{frame}
  \begin{corollary}[Application]
    ...
    \]
  \end{corollary}
  Observe that $\vec{d} = (\sem{\Gamma \vdash x_1 : \sigma_1}\;\vec{d}, \ldots,
  \sem{\Gamma \vdash x_n : \sigma_n}\;\vec{d})$ for any context
  $\Gamma = x_1 : \sigma_1, \ldots, x_n : \sigma_n$. Then, this corollary
  is a series of simple facts:
    \begin{align*}
      ...
      = {} & 
        \mathit{ev}
        \left(
          \sem{\Gamma, x : \sigma \vdash \M : \tau}\;\vec{d}, 
          \sem{\Gamma \vdash \N : \sigma}\;\vec{d}
        \right)\\
        ={} & 
...
\begin{frame}
  \begin{corollary}[Extensionality]
